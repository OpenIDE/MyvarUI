...
namespace MyvarUI.SDL
{
    public unsafe class LinuxSdl : ISDL
    {
        ...
        public static string SLDLIbName = "libSDL2.so";
        ...
        {
            var init = TTF_Init();
            if (SDL_Init((UInt32)SdlInit.SDL_INIT_VIDEO) < 0)
            {
                DumpErrors();
        ...
        public void DrawText(string text, int x, int y, string font, int sizept, Color c)
        {
            ...
            var afont = TTF_OpenFont(Path.GetFullPath("./Fonts/SourceCodePro.ttf"), sizept);

            var surface = TTF_RenderText_Solid(afont, text, c.ToStruct());
            ...
            var res = SDL_RenderCopy(renderer, texture, null, &rect);
		...
		public Size CalulateTextSize(string text, string font, int sizept)
		{
            ...
            var surface = TTF_RenderText_Solid(afont, text, Color.White.ToStruct());
            SDL_Surface WHRef = (SDL_Surface)Marshal.PtrToStructure(new IntPtr(surface), typeof(SDL_Surface));
         ...
         public char[] KeyPresses()
         {
             ...
             int lenth = 0;
             var state = SDL_GetKeyboardState(ref lenth);
        ...
        public MouseState GetMouseState()
        {
            ...
            var mask = SDL_GetMouseState(ref x, ref y);
        ...
        static extern byte* SDL_GetKeyboardState(ref int numkeys);


        [DllImport("libSDL2.so")]
        static extern uint SDL_GetMouseState(ref int x, ref int y);


        [DllImport("/usr/local/lib/libSDL2_ttf.so")]
        static extern void* TTF_RenderGlyph_Solid(void* font, ushort ch, ColorStruct fg);

        [DllImport("libSDL2.so")]
        static extern int SDL_BlitSurface(void* src, void* srcrect, void* dst, void* dstrect);


        [DllImport("libSDL2.so")]
        static extern void* SDL_RWFromFile([MarshalAs(UnmanagedType.LPStr)] string file, [MarshalAs(UnmanagedType.LPStr)] string mode);

        [DllImport("libSDL2.so")]
        static extern int SDL_SaveBMP_RW(void* surface, void* io, int freedst);



        [DllImport("libSDL2.so")]
        static extern int SDL_SaveBMP(void* surface, string file);

        [DllImport("libSDL2.so")]
        static extern int SDL_SetRenderTarget(void* renderer, void* texture);


        [DllImport("libSDL2.so")]
        static extern int SDL_RenderCopy(void* renderer, void* texture, SDL_Rect* srcrect, SDL_Rect* dstrect);

        [DllImport("libSDL2.so")]
        static extern string SDL_GetBasePath();

        [DllImport("/usr/local/lib/libSDL2_ttf.so")]
        static extern int TTF_Init();

        [DllImport("/usr/local/lib/libSDL2_ttf.so")]
        static extern void* TTF_RenderText_Shaded(void* font, [MarshalAs(UnmanagedType.LPStr)] string text, ColorStruct fg, ColorStruct bg);

        [DllImport("libSDL2.so")]
        static extern void* SDL_CreateTextureFromSurface(void* renderer, void* surface);

        [DllImport("libSDL2.so")]
        static extern short* SDL_GetError();

        [DllImport("/usr/local/lib/libSDL2_ttf.so")]
        static extern void* TTF_RenderText_Solid(void* font, [MarshalAs(UnmanagedType.LPStr)] string text, ColorStruct fg);

        [DllImport("/usr/local/lib/libSDL2_ttf.so")]
        static extern void* TTF_OpenFont(string filename, int ptsize);

        [DllImport("libSDL2.so")]
        static extern int SDL_Init(UInt32 flags);

        [DllImport("libSDL2.so")]
        static extern int SDL_CreateWindowAndRenderer(int width, int height, uint window_flags, void** window, void** renderer);

        [DllImport("libSDL2.so")]
        static extern int SDL_RenderDrawPoint(void* renderer, int x, int y);

        [DllImport("libSDL2.so")]
        static extern void SDL_RenderPresent(void* renderer);

        [DllImport("libSDL2.so")]
        static extern void SDL_SetWindowTitle(void* window, string title);

        [DllImport("libSDL2.so")]
        static extern int SDL_SetRenderDrawColor(void* renderer, byte r, byte g, byte b, byte a);

        [DllImport("libSDL2.so")]
        static extern int SDL_RenderClear(void* renderer);


        [DllImport("libSDL2.so")]
        static extern int SDL_PollEvent(byte* aEvent);

        [DllImport("libSDL2.so")]
        static extern void SDL_DestroyRenderer(void* renderer);

        [DllImport("libSDL2.so")]
        static extern void SDL_DestroyWindow(void* renderer);

        [DllImport("libSDL2.so")]
        static extern void SDL_Quit();

        [DllImport("libSDL2.so")]
        static extern void* SDL_CreateRenderer(void* window, int index, uint flags);

        [DllImport("libSDL2.so")]
        static extern void* SDL_CreateWindow(string title, int x, int y, int w, int h, uint flags);
